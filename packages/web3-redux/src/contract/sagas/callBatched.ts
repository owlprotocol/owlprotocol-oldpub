...
import { CallBatchedAction, call as callAction } from '../actions/index.js';
import { ContractWithObjects } from '../model/interface.js';
import { callSaga } from './call.js';

export function* callBatched(action: CallBatchedAction): Generator<
    any,
    {
...
> {
    ...
    const { calls } = payload;
    const callSagaResults = yield* all(calls.map((c) => {
        return call(callSaga, callAction(c, action.meta.uuid))
    }))

    return callSagaResults;
