...
export const ERC721GenerativeImage = (props: ERC721GenerativeImageProps) => {
    ...
    const [nftOnchain] = NFTGenerativeItem.hooks.useGenerativeItemOnchain(status == 'onchain' ? networkId : undefined, status == 'onchain' ? address : undefined, tokenId)
    const [nftLocal] = NFTGenerativeItem.hooks.useGenerativeItemLocal(status == 'draft' ? networkId : undefined, status == 'draft' ? address : undefined, tokenId);
