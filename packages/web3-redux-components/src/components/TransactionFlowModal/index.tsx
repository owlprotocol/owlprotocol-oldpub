...
const StatusIcon = ({ icon }: any) => {
    if (icon === 1 || icon === 2)
        return <CardIcon style={{ width: "100%", height: "100%" }} />;
    else if (icon === 3 || icon === 4)
        return <InProgressIcon style={{ width: "100%", height: "100%" }} />;
    else if (icon === 5)
        return <ConfirmedIcon style={{ width: "100%", height: "100%" }} />;
    else return <InProgressIcon style={{ width: "100%", height: "100%" }} />;
};

const TransactionFlowModal = ({
    isOpen,
    tokenName,
    stage,
    actionHandler,
    ...
    const TXFlowLabels = [
        ...
        "Receive allowance confirmation",
        "Swap tokens",
        "Confirmation",
    ];
    ...
    return (
        <>
            <Modal {...settings}>
                <ModalOverlay />
                <ModalContent bg={themes.color5} borderRadius={8}>
                    <ModalBody>
                        <Box
                            maxW={540}
                            h={100}
                        ...
                        <Box
                            ...
                            textAlign={"center"}
                        >
                            Please confirm the proposed {tokenName} spend
                            allowance in wallet
                        </Box>
                    </ModalBody>
                    <ModalFooter>
