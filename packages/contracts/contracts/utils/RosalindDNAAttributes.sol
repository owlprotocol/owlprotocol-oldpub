...
library RosalindDNAAttributes {
     ...
     * @param isBreedableAttribute only breed for attributes
     * which are marked as true for this field. Corresponds with the gene
     * configuration.
    ...
    function breedDNASimple(
        ...
        uint8[] memory genes,
        bool[] memory isBreedableAttribute,
        uint256 randomSeed
    ) internal pure returns (uint256 childDNA) {
        require(isBreedableAttribute.length + 1 == genes.length, 'isBreedableAttribute.length + 1 != genes.length');

        // Loop genes
        for (uint16 geneIdx = 0; geneIdx < genes.length - 1; geneIdx++) {
            if (isBreedableAttribute[geneIdx] != true)
                // Skip if not a breedable attribute
                continue;

            // Gene details
            uint16 geneStartIdx = genes[geneIdx];
            uint16 geneEndIdx = genes[geneIdx + 1];
    ...
    function breedDNAWithMutations(
        ...
        // Breed normally
        childDNA = breedDNASimple(parentsDNA, genes, isBreedableAttribute, randomSeed);

        /**
         * @note it makes sense to not necessarily pass isBreedableAttribute to
         * mutation generation. That way, you can have genes which don't breed
         * but CAN still mutate. If you want to disable mutations AND breeding
         * for a gene, isBreedableAttribute should be set to false AND
         * mutationRate should be 2^256-1.
         */
    ...
    function generateMutations(
        ...
        for (uint256 geneIdx = 0; geneIdx  - 1 < genes.length; geneIdx++) {
